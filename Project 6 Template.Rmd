...
for(sim in 1:n_sims){
    ...
    ATT <- as.data.frame(append(ATT, ATT_NN))
    ...
    beats_threshold <- as.data.frame(append(beats_threshold, thresh_prop))
        
    }
    
ATT <- rename(ATT, attnn = "append(ATT, ATT_NN)")
ATT <- tibble::rowid_to_column(ATT, "ID")

beats_threshold <- rename(beats_threshold, thresbt = "append(beats_threshold, thresh_prop)")
...
n_sims <- 1000 #LL note on 4/4: just doing 1000 for now, but we can up this and hopefully won't break
ATTam <- c()
...
# Fit  models and save ATTs, proportion of balanced covariates, and mean percent balance improvement
    ...
    ATTam <- append(ATT, ATT_mahal)

    #calculate mean percent improvement
    reduction_df <- as.data.frame.matrix(full_summary$reduction)
    reduction_mean <- mean(reduction_df$`Std. Mean Diff.`)
    percent_improvements <- as.data.frame(append(percent_improvement, reduction_mean))
    ...
    threshs_props <- (thresh_count / nrow(matched_df2))*100
    
    beats_thresholds <- append(beats_thresholds, thresh_prop)
...
ATTm <- rename(ATT, attnn = "append(ATT, ATT_NN)")
ATTm <- tibble::rowid_to_column(ATT, "ID")

beats_thresholds <- rename(beats_thresholds, thresbt2 = "append(beats_threshold, thresh_prop)")

beats_thresholds <- tibble::rowid_to_column(beats_threshold, "ID")


# Plot ATT v. proportion

mergedatt2 <- merge(x = ATTm, y = beats_threshold, by="ID", all.x=TRUE)

ggplot(mergedatt2, aes(x = attnn, y = thresbt)) + 
  geom_point() +
  labs(x = "ATT", y = "Threshold") +
  stat_summary(fun.data= mean_cl_normal) + 
  geom_smooth(method='lm')
...
percent_improvements <- rename(percent_improvements, prcimp = "append(percent_improvement, reduction_mean)")

percent_improvements %>% ggplot(aes(prcimp)) +
